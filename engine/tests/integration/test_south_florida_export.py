...
def test_south_florida_export_contains_thirty_four_objective_cases(tmp_path) -> None:
    ...
    assert len(routes["features"]) == 120
    assert {item["algorithm"] for item in metrics["results"]} == {
        "shortest",
        "fastest",
        "energy",
        "balanced",
