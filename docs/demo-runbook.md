# Professor demonstration runbook

1. Run `npm ci`, `uv sync --frozen --project engine`, `npm run check`, and `npm run test:e2e` before travel.
2. Verify the pinned artifact with `uv run --project engine aquanav data validate --scenario south-florida-noaa-v1`.
3. Start `npm run dev -- --host 127.0.0.1` and open `http://127.0.0.1:5173`.
4. Begin with the research-only warning and NOAA-derived provenance boundary.
5. Compare the four objectives and explain legitimate fastest/energy overlap.
6. Play, pause, resume, seek, switch route, and reset the visible ASV marker.
7. Click two offshore points and show four new routes without global revalidation.
8. Open provenance and explain the static-web/local-engine boundary, proxy energy, 500 m limitations, deferred ENC hazards, and excluded waves.
9. For the technical deep dive, show the common graph, deterministic tests, 30 cases/120 routes, worker protocol, and learned-model evidence gate.

Never describe a route as safe or operational, and never describe modelled energy as measured performance.
